...
def create_booking_from_metadata(reference, metadata, status):
    existing = Booking.query.filter_by(reference=reference).first()
    if existing:
        return existing

    booking_date = datetime.strptime(metadata["date"], "%Y-%m-%d").date()
    start_time = datetime.strptime(metadata["start_time"], "%H:%M").time()
    hours = int(metadata["hours"])
    start_dt = datetime.combine(booking_date, start_time)
    end_time = (start_dt + timedelta(hours=hours)).time()
...
def paystack_webhook():
    ...
    if event.get("event") == "charge.success":
        ...
        try:
            create_booking_from_metadata(reference, metadata, "Confirmed")
            logger.info(f"Booking {reference} created as Confirmed via webhook.")
        except Exception as e:
            logger.error(f"Failed to create booking from webhook metadata for {reference}: {e}")
...
def verify_payment(reference):
    existing = Booking.query.filter_by(reference=reference).first()
    if existing:
        return jsonify({"status": existing.status, "reference": reference})
    ...
    if not metadata:
        return jsonify({"error": "No booking data found for this transaction."}), 404

    status = "Confirmed" if result["status"] == "success" else "Failed"

    try:
        create_booking_from_metadata(reference, metadata, status)
    except Exception as e:
        logger.error(f"Failed to create booking from verify metadata for {reference}: {e}")
        return jsonify({"error": "Could not finalize booking record."}), 500

    return jsonify({"status": status, "reference": reference})
